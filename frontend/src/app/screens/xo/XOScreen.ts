import { Color, Container, FederatedPointerEvent, Graphics, Point, Rectangle, Ticker } from "pixi.js";
...
class XoBoard extends Container {
	...
	constructor(gridSize: number) {
		...
		this.onmouseup = (event: FederatedPointerEvent) => {
			...
			// Switch turn
			this.currentPlayer = this.currentPlayer === CellType.XCell ? CellType.OCell : CellType.XCell;
		};
	}
	...
	private drawSymbol(row: number, col: number, symbol: CellType) {
		...
		if (symbol === CellType.XCell) {
			...
			const radius = size / 2;
			this.cellsGraphics.circle(centerX, centerY, radius)
